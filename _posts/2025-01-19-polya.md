...
可以证明此运算符具有类似结合律的性质，也就是说 $(g\circ f)*\mathbf c=g*(f*\mathbf c)$ **恒**成立。
...
称两种染色 $\mathbf{c_1}\sim\mathbf{c_2}$，当且仅当存在 $f\in G,f*\mathbf{c_1}=\mathbf{c_2}$。  
...
定理 $2.1$：$g*\mathbf c=f*\mathbf c$ 当且仅当 $f^{-1}\circ g\in G(\mathbf c)$。
