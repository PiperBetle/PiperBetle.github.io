...
置换是一种变换，如果把 $\rho_n$ 看成多边形 $360\degree/n$ 旋转，那么 $\rho_n^k$ 就是 $k(360\degree/n)$ 旋转。如何理解？
...
为 $X$ 上的置换。  
定义 $f*\bm c$ 为使得 $i_k$ 具有颜色 $c(k)$ 的着色，即

$$
(f*\bm c)(i_k)=c(k),k\in[n]
$$

可以证明 $*$ 具有类似结合律的性质，也就是说 $(g\circ f)*\bm c=g*(f*c)$ **恒**成立。

举个例子，如果在正方形上每个点可以染红色或者蓝色，按 $1,2,3,4$ 的顺序列举顶点的颜色。  
例如着色 $\bm c=(R,B,B,R)$ 就指顶点 $1,2,3,4$ 的颜色分别为红蓝蓝红。  
如果着色 $\bm c$ 加以 $\rho_4$ 就把着色循环「右」移一位，变成 $(R,R,B,B)$。  
如果着色 $\bm c$ 加以 $\tau_3$ 就相当于 $1,2$ 相互对称，$3,4$ 相互对称，变成 $(B,R,R,B)$。  
如果着色 $\bm c$ 加以 $\tau_4$ 就相当于 $1,4$ 相互对称，$2,3$ 相互对称，变成 $(R,B,B,R)$。  
可以看出 $\bm c$ 加以 $\tau_4$ 后并没有改变着色。
...
- $\forall f\in G,\forall\bm c\in\mathcal C,f*\bm c\in\mathcal C$

$\mathcal C$ 的定义依赖于置换群 $G$，但我们的记号里并没有显示出来，因为一般情况下这不会引起误解。

称两种染色 $\bm{c_1}\sim\bm{c_2}$，当且仅当 $\exist f\in G,f*\bm{c_1}=\bm{c_2}$。  
经典的，我们可以得出 $\sim$ 的几个性质

1. 自反性，$\forall\bm c\in\mathcal C,\bm c\sim\bm c$；
1. 对称性，$\bm{c_1}\sim\bm{c_2}\implies\bm{c_2}\sim\bm{c_1}$；
1. 传递性，$\bm{c_1}\sim\bm{c_2},\bm{c_2}\sim\bm{c_3}\implies\bm{c_1}\sim\bm{c_3}$。

$\sim$ 的定义依赖于置换群 $G$，但我们的记号里并没有显示出来，因为一般情况下这不会引起误解。

令 $G$ 是 $X=[n]$ 的置换群，$\mathcal C$ 是 $X$ 的着色集，且 $\mathcal C$ 的定义依赖于置换群 $G$。

定义 $G(\bm c)$ 表示所有置换 $f$，满足 $f*\bm c=\bm c$ 组成的集合，即

$$G(\bm c)=\{f:f\in G,f*\bm c=\bm c\}$$

我们称 $G(\bm c)$ 为着色 $\bm c$ 的稳定核。

定义 $\mathcal C(f)$ 表示所有着色 $\bm c$，满足 $f*\bm c=\bm c$ 组成的集合，即

$$\mathcal C(f)=\{\bm c:\bm c\in\mathcal C,f*\bm c=\bm c\}$$

可以证明任意着色的稳定核是个置换群。

定理 $2.1$：$g*\bm c=f*\bm c\iff f^{-1}\circ g\in G(\bm c)$。

证明：可以同乘 $f^{-1}$，再使用结合律证明左推右。倒着写就是右推左。<p align="right" style="font-size:1.25rem;">■</p>

推论 $2.2$：与 $\bm c$ 等价的着色数等于 $G$ 中的置换个数除以 $\bm c$ 的稳定核中的置换个数，即

$$\vert\{f*\bm c:f\in G\}\vert=\frac{\vert G\vert}{\vert G(\bm c)\vert}$$

证明：由定理 $2.1$，$g*\bm c=f*\bm c\iff f^{-1}\circ g\in G(\bm c)\iff g\in\{f\circ h:h\in G(\bm c)\}$。  
记集合 $s=\{f\circ h:h\in G(\bm c)\}$，取 $h_1,h_2\in s$，记 $g_1=f\circ h_1$，$g_2=f\circ h_2$。  
根据 $G(\bm c)$ 的定义，$h_1*\bm c=h_2*\bm c\implies f\circ h_1*\bm c=f\circ h_2*\bm c\implies g_1*\bm c=g_2*\bm c$。  
因为 $g$ 互不相同，所以对于每个置换 $f$，恰好存在 $\vert G(\bm c)\vert$ 个置换，这 $\vert G(\bm c)\vert$ 个置换作用在 $\bm c$ 上有相同的效果。  
所以将 $\vert G\vert$ 除以 $\vert G(\bm c)\vert$ 即证。<p align="right" style="font-size:1.25rem;">■</p>
...
考虑计数如下二元组 $(f,\bm c)$ 的数量，满足 $f*\bm c=\bm c$，那么

$$
\sum_{f\in G}\vert\mathcal C(f)\vert=\sum_{\bm c\in\mathcal C}\vert G(\bm c)\vert
$$

由推论 $2.1$ 可得

$$
\vert G(\bm c)\vert=\frac{\vert G\vert}{\#[与 \bm c 等价的着色数]}
$$

因此

$$
\sum_{\bm c\in\mathcal C}\vert G(\bm c)\vert=\vert G\vert\sum_{\bm c\in\mathcal C}\frac1{\#[与 \bm c 等价的着色数]}
$$

如果有 $k$ 种着色等价，那么每种着色的贡献都是 $1/k$，也就是说每种本质不同的着色数的贡献均为 $1$。因此

$$
\sum_{\bm c\in\mathcal C}\frac1{\#[与 \bm c 等价的着色数]}=N(G,\mathcal C)
$$

联立上述两式稍加整理，即证。<p align="right" style="font-size:1.25rem;">■</p>
...
$f*\bm c$ 相当于把 $1$ 的颜色给了 $6$，把 $6$ 的颜色给了 $3$，把 $3$ 的颜色给了 $5$，把 $5$ 的颜色给了 $1$；把 $2$ 的颜色给了 $8$，把 $8$ 的颜色给了 $7$，把 $7$ 的颜色给了 $2$；把 $4$ 的颜色给了 $4$。  
