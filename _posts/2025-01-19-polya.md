...
为 $X$ 上的置换。  
定义 $f*\mathbf c$ 为使得 $i_k$ 具有颜色 $c(k)$ 的着色，即

$$
(f*\mathbf c)(i_k)=c(k),k\in[n]
$$

可以证明 $*$ 具有类似结合律的性质，也就是说 $(g\circ f)*\mathbf c=g*(f*c)$ **恒**成立。
...
- $\forall f\in G,\forall\mathbf c\in\mathcal C,f*\mathbf c\in\mathcal C$
...
定义 $G(\mathbf c)$ 表示所有置换 $f$，满足 $f*\mathbf c=\mathbf c$ 组成的集合，即

$$G(\mathbf c)=\{f:f\in G,f*\mathbf c=\mathbf c\}$$

我们称 $G(\mathbf c)$ 为着色 $\mathbf c$ 的稳定核。

定义 $\mathcal C(f)$ 表示所有着色 $\mathbf c$，满足 $f*\mathbf c=\mathbf c$ 组成的集合，即

$$\mathcal C(f)=\{\mathbf c:\mathbf c\in\mathcal C,f*\mathbf c=\mathbf c\}$$

可以证明任意着色的稳定核是个置换群。

定理 $2.1$：$g*\mathbf c=f*\mathbf c\Longleftrightarrow f^{-1}\circ g\in G(\mathbf c)$。
...
$$\vert\{f*\mathbf c:f\in G\}\vert=\frac{\vert G\vert}{\vert G(\mathbf c)\vert}$$

证明：由定理 $2.1$，$g*\mathbf c=f*\mathbf c\Longleftrightarrow f^{-1}\circ g\in G(\mathbf c)\Longleftrightarrow g\in\{f\circ h:h\in G(\mathbf c)\}$。  
记集合 $s=\{f\circ h:h\in G(\mathbf c)\}$，取 $h_1,h_2\in s$，记 $g_1=f\circ h_1$，$g_2=f\circ h_2$。  
根据 $G(\mathbf c)$ 的定义，$h_1*\mathbf c=h_2*\mathbf c\implies f\circ h_1*\mathbf c=f\circ h_2*\mathbf c\implies g_1*\mathbf c=g_2*\mathbf c$。  
...
考虑计数如下二元组 $(f,\mathbf c)$ 的数量，满足 $f*\mathbf c=\mathbf c$，那么
...
$f*\mathbf c$ 相当于把 $1$ 的颜色给了 $6$，把 $6$ 的颜色给了 $3$，把 $3$ 的颜色给了 $5$，把 $5$ 的颜色给了 $1$；把 $2$ 的颜色给了 $8$，把 $8$ 的颜色给了 $7$，把 $7$ 的颜色给了 $2$；把 $4$ 的颜色给了 $4$。  
